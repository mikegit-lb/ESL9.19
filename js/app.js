...
(() => {
  ...
  function getLoadedQuestions() {
    if (typeof window !== 'undefined' && Array.isArray(window.ESL_QUESTIONS) && window.ESL_QUESTIONS.length > 0) {
      return window.ESL_QUESTIONS;
    }
    if (typeof globalThis !== 'undefined' && Array.isArray(globalThis.ESL_QUESTIONS) && globalThis.ESL_QUESTIONS.length > 0) {
      return globalThis.ESL_QUESTIONS;
    }
    if (typeof ESL_QUESTIONS !== 'undefined' && Array.isArray(ESL_QUESTIONS) && ESL_QUESTIONS.length > 0) {
      return ESL_QUESTIONS;
    }
    return [];
  }

  function getGrammarCategories() {
    if (typeof window !== 'undefined' && window.GRAMMAR_CATEGORIES && Object.keys(window.GRAMMAR_CATEGORIES).length > 0) {
      return window.GRAMMAR_CATEGORIES;
    }
    if (typeof globalThis !== 'undefined' && globalThis.GRAMMAR_CATEGORIES && Object.keys(globalThis.GRAMMAR_CATEGORIES).length > 0) {
      return globalThis.GRAMMAR_CATEGORIES;
    }
    if (typeof GRAMMAR_CATEGORIES !== 'undefined' && Object.keys(GRAMMAR_CATEGORIES).length > 0) {
      return GRAMMAR_CATEGORIES;
    }
    return {};
  }

  let allQuestions = [...getLoadedQuestions(), ...customQuestions];
  ...
  function renderCurrentCard() {
    ...
    const catMeta = getGrammarCategories()[q.grammar] || {};
  ...
  function renderGrammarGuide() {
    const categoriesMap = getGrammarCategories();
    if (!elements.cheatSheetsGrid || Object.keys(categoriesMap).length === 0) return;
  ...
  function handleCustomPromptSubmit(e) {
    ...
    const newQuestion = {
      id: `custom-${Date.now()}`,
      grammar,
      categoryLabel: getGrammarCategories()[grammar]?.label || grammar,
      title,
      question,
      followUps,
      level: "B2",
  ...
  // =========================================================================
  function init() {
    const base = getLoadedQuestions();
    allQuestions = [...base, ...customQuestions];
    filteredQuestions = [...allQuestions];
    initTheme();
    updateBadgeCounts();
    renderCurrentCard();
    ...
    // Fallback retry in case of delayed script evaluation
    if (allQuestions.length === 0) {
      setTimeout(() => {
        const retryBase = getLoadedQuestions();
        if (retryBase.length > 0) {
          allQuestions = [...retryBase, ...customQuestions];
          filteredQuestions = [...allQuestions];
          updateBadgeCounts();
          renderCurrentCard();
          renderGrammarGuide();
